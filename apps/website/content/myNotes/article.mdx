...
Consider this <Emoji name="arrow-big-up" xl important fill/> diagram, in which a body is in motion perpendicular to the normal vector at the surface of the Earth. As $\dot{x} = dx/dt$, we can describe $\dot{x}~ dt$ as just $\dot{x}$. This leaves us with
