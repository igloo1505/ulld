...
This means that for every infinitely small piece of space, we would need to scale that space by a factor of $\delta$ each second to produce our surface gravitational acceleration of $g$.
...
<Admonition icon={false} title="In regards to $\bar{v}$" type="info" dropdown open>

Unless a reference to pseudo-time or a pseudo-time derivative is included in the following equations explicitly, when $\bar{v}$ appears as a constant it is referring to the time independent quantity as:
$$
\begin{align}
    \bar{v} = c \sqrt{ 1 - \frac{1}{\left( 1 + G \frac{M_{\oplus}}{r^{3}} \right)^{2}}} = 525,820.603 ~ \frac{\text{m}}{\text{s}}
\end{align}
$$
...
If we consider this experiment in the context of the time equivalent quantities inherent to the model being proposed, we should find that
...
$$
\begin{align}
\bar{\gamma} = \frac{R_{1}}{R_{0}} = \frac{R_{0} \left( 1 + \int \int_0^{R_{\oplus}} 2 G \frac{M_{\oplus}}{R^{3}_{\oplus}} \frac{1}{\bar{v}} \tau_\epsilon ~ dR ~ d \tau_\epsilon \right)}{R_{0}}
\end{align}
$$
...
Consider the diagram above, in which a body is in motion perpendicular to the normal vector at the surface of the Earth. As $\dot{x} = dx/dt$, we can describe $\dot{x}~ dt$ as just $\dot{x}$. This leaves us with
...
If we then substitute our standard orbital velocity formula for $\dot{x}$ and allow $\lim_{dt_p \to 0}$, we find
...
Consider the diagram in figure <FigRef id="dsPhiRelationship" /> above. Recall that in the model being proposed, proportional velocity derivatives should be proportional to some $dx_c$ velocity derivative of the speed of light. This is to say that for some infinitesimally small $dx_c$, $dx_v = dx_c \frac{v}{c}$.
