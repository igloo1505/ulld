...
Given this interpretation of velocity and time, we might review figure <FigRef id="space-dilation-phi"/> with a loosened dependence on time. If we consider $v$ as a pure magnitude as

$$
\begin{align}
v = \bar{v} = c \sqrt{1 - \frac{1}{\left( 1 + \frac{g}{R} \right)^{2}}}
\end{align}
$$
...
It is my position that $\bar{v}$ should still be considered a derivative, $dx$, and this magnitude of $526.6 ~ \text{km} ~ \text{s}^{-1}$ be applied only as a dimensionless scalar; as a means of comparison to the speed of light, $c$.
...
## interpretation
<Small noMargin muted>The [Relativistic Symmetry](/blog/physics/relativisticEquivalence) post describes the relationship between the symmetry described here and another symmetry that arises from our rotational velocity.</Small>
...
However, note the difference between $ds$ described here and $ds$ as described in the [Relativistic Symmetry](/blog/physics/relativisticEquivalence) post. The behavior of $ds$ varies with respect to reference frame. In the case described here, in which $ds$ is the consequence of a single body moving through space, from the reference frame of that body, this $ds ~ dx = \Phi$ symmetry applies, as the body itself is traversing this additional distance $ds$. 

In the case described in the first section of the [Relativistic Symmetry](/blog/physics/relativisticEquivalence) post, where the observer is not the source of the dilation and is not traversing the additional $ds$, the observer experiences the $ds$ as gravity, and another symmetry applies as described in that article.
