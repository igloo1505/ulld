...
description: "The theory of gravitational time and divergent coordinate systems will undoubtedly be controversial before it becomes accepted. Covered here are 15 predictions that follow from this model of time and relative geometry that are unaccounted for by existing relativity based models."
...
As this model predicts a Universe in which all $\mathbb{R}^{3}$ spatial coordinates exist concurrently along this density axis, there is no temporal limitation on the evolution of distant galaxies. This model produces no mechanism that dictates that our 'Big Bang' event must have happened concurrently with that of another distant observer. When the world line of Earth was at a temporal position that we now describe as the big bang, there is no reason that other bodies couldn't exist at a temporal position that even now is in the 'future' according to our reference frame.
...
> See [Center of Mass Shift](blog/physics/centerOfMassShift)
...
$$
\begin{align}
  \int_{0}^{R} ds ~ dr ~ dt = 2 G \frac{M}{R^3} ~ dt
\end{align}
$$
...
As first addressed by Born in 1909<span className="citationAnchor-container no-tooltip"><a href='#bib-bornGravityChargedParticleParadox' className="citation citationAnchor" id="cit-bornGravityChargedParticleParadox-idx-4">5</a></span>, why should it be that an accelerated charged particle does not produce electromagnetic radiation when accelerated by a gravitational field in the same manner as when it is being accelerated in $\mathbb{R}^{3}$ space? After all, the equivalence principle dictates that these two accelerated reference frames should be entirely indistinguishable to a local observer.

If the electromagnetic field exists as medium within a larger $\mathbb{R}^{3}$ space, $\mathbb{R}^{3}_\tau$ that encompasses our expanding Universe, and it is the local spatial dilation that is in motion relative to this larger space, $\mathbb{R}^{3}_{\tau}$, the particle in a gravitationally accelerated reference frame would be at rest with respect to the larger $\mathbb{R}^{3}_{\tau}$ space while space itself is in motion. This further supports the mass independence of gravity discussed [here](#gravity-mass-independence), and lends further credence to the notion that gravity is not a force.
