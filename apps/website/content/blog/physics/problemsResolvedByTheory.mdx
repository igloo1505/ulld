...
The model of spatial dilation as the mechanism of gravity predicts _exactly_ the lensing effects regarding the bullet cluster. If two or more bodies of significant mass and velocity approach each other, this spatial dilation eventually interfere with the dilation of the other approaching bodies, which in turn would stop or slow this dilation's outward flow. The result would be _exactly_ what we see: A gravitational lens flare that does not align with the center of observable mass.

### 2. Relativistic Doppler Shift Without Naively Dilating Time

While the Newtonian Doppler shift follows from pure motion in a way that aligns perfectly with our observations of the Universe around us, relativistic Doppler shifts require that we dilate the time elapsed for the user. See [Consistent Simultaneity](#consistent-simultaneity) for why this is nonsensical. If the passage of this mysterious medium we describe as time does not dilate with respect to relative motion, but it is distance that dilates and in turn the dilation of this distance is what we experience as time, we can achieve preciously the same experimental results without applying $\gamma$ as a scalar of a quantity that we can not directly observe.

Consider the 1959 Pound-Rebka experiment<span className="citationAnchor-container no-tooltip"><a href='#bib-pound_rebka' className="citation citationAnchor" id="cit-pound_rebka-idx-0">1</a></span> in which researchers at Harvard placed an emitter at $h \approx 22.5 ~ \text{meters}$ and attempted to find the relative velocity away from the receiver at $h = 0$[^1] that would perfectly cancel the gravitational Doppler shift of the light emitted from the additional height $h$. They found a velocity of $v = g \frac{h}{c} = g \Delta t$, which coincides precisely with what is predicted by a model in which spatial dilation occurs at exactly $g$ at distance $R$, where $h$ is the height _above_ $R$.


$$
\begin{align}
 \int_0^{h/c} \dot{x} ~ dt = \int_0^{h/c} \left(\begin{array}{ccc}
     \int_0^{R} 2 G \frac{m}{R^{3}}
 & 0 & 0 \\
     0 &  \int_0^{R} 2 G \frac{m}{R^{3}}
 & 0 \\
     0 & 0 &  \int_0^{R} 2 G \frac{m}{R^{3}} 
    \end{array}\right)  \hat{R} ~ dt
\end{align}
$$
...
$\Phi$ has long been one of, or perhaps the single most physically consequential transcendental quantity. The fact that $ds = \bar{v} \delta = 1.61800 = \Phi \pm 0.002\%$ suggests some sort of gravitational, or perhaps even unified well. If we exist at a density and velocity such that $g$ and $v$ appear to conspire in some unknown manner to produce this transcendental ratio with such unique mathematical properties, it could be that this is the natural course of the Universe. As all of what we know about the Universe indicates that we are not unique or special, it could be that other forms of complex life exist across not only vast spatial distances, but also across varied densities such that their own velocity, density and gravitational acceleration conspire in a similar manner.



### 7. The Mass Independence of Gravity [#gravity-mass-independence]

This model relies upon a mechanism of gravity in which the kinetic state of the observer does not change in response to what we observe as gravitational acceleration. If it is space the dilates toward the observer while the observer remains in their inertial state, then $\ddot{x} = 0$ and $ma = 0$. Since the observer or body experiencing this dilation of space experiences no acceleration, $F_{g} = 0$ and gravity is indeed not a force. Not only does this directly address the struggle to quantify gravity, but this describes a mechanism of action in which the mass of the observer ***must*** be irrelevant to the action of the effects of this dilating space in any direct manner.

### 8. Equivalence Principle

The model of relative spatial dilation being proposed suggests that the equivalence principle is far more than a useful mathematical artifact. Since the velocity of an observer in free fall due to gravity is found by:
...
We can infer, according to this model of spatial dilation, that the equivalence principle does not describe an equivalence between gravity and an upward _accelerating_ platform, but rather an equivalence between gravity and a consistent upward motion along a dimension in which the inertia of a body in $\mathbb{R}^{3}$ space is not conserved. This could be due to a mechanism akin to a buoyant force, in which motion along this density axis that exceeds that of the surrounding spatial dilation is met by resistance, or perhaps this is a feature of this 4-dimensional geometry itself. Regardless, if the acceleration in $\mathbb{R}^{3}$ space of body $b$ that appears secondarily to the dilation of space inside of another 3 dimensional body $a$ produces an acceleration under which the additional velocity of $b$ does not produce inertia in $\mathbb{R}^{3}$ space, gravity produces effects that mirror exactly what is observed, without the need for an ***accelerating*** platform. Note that this lack of $\mathbb{R}^{3}$ inertia due to acceleration that is secondary to spatial dilation is consistent with the principle that the motion of body $b$ relative to the underlying coordinate system is $0$.

### 9. Michelson & Morley
<Small noMargin muted style={{position: "absolute"}} y={-16}>Correlation, not prediction.</Small>

Michelson and Morley<span className="citationAnchor-container no-tooltip"><a href='#bib-michelson_morely' className="citation citationAnchor" id="cit-michelson_morely-idx-1">2</a></span> are often credited with disproving the existence of a luminous ether by demonstrating that the speed of light does not vary according to the direction of the emission of this light, ***across*** the Earth's surface. The belief was that if a luminous ether existed, the motion of Earth through this ether would produce measurable drag in the direction of travel. When examined closer however, this experiment while incredibly clever, merely demonstrates the lack of drag produced by motion through space. If we consider the principle of spatial dilation being proposed, this ether, or space itself dilates in a radiating pattern from massive bodies as they move through space. Not only does this dilation of space in a radiating pattern not conflict with the Michelson Morley experiment, it predicts _precisely_ the gravitational Doppler shift of light that we've confirmed experimentally.

### 10. Retro-causality

If an action is taken at $\left( x, ~ y, ~ z, ~ s \right)$ there is no reason to conclude that that action can not have an affect at $\left( x + n, ~ y, ~ z, s + \tau \right)$, and have another affect at $\left( x + n - 1, ~ y, ~ z, ~ s + \tau - 1 \right)$, in that order, before the observer arrives at $s + \tau - 1 + \frac{x + n - 1}{c}$. This model predicts a mechanism of chronological ordering that allows time to behave as another spatial axis in the sense that events may occur in an order along this axis that is not necessarily linear. Not only does this provide a mechanism for retrocausality, permitting that all observations are in the 'future' relative to the action that initiated that sequence of events if the force carrying mechanism can act across this density axis, but many time travel paradoxes no longer apply.


### 11. Lack of Gravitational Aberration

As this model of relative spatial dilation declares that this dilation of space ***is*** time, we can easily infer that objects that appear to share a time component of their displacement 4-vector may exist separated by some density. As the dilation of space being proposed increases as a function of the magnitude of motion _through_ space, it must be that less dense coordinate systems exist in the 'future' with respect to an observer at a greater density. As this dilation of space is proportional to $g$, a decrease in $g$ should correspond with a decrease in density, $s$.  

In turn, if we extrapolate upon this model to describe a system similar to our own solar system, in which a vastly more massive body is orbited by one or more less massive bodies, the more massive body would exist at a greater density at the time of the emission of this spatial dilation, despite appearing to exist at a shared density to the orbiting bodies. This discrepancy in density would supersede the $\Delta t = \frac{d}{v_{g}}$ period expected, where $v_{g}$ is the theoretical velocity of gravity itself, to include this discrepancy in density as well. If gravity exerts itself along this temporal component of a $\mathbb{R}^{4}$ spatial system, and the orbiting body exists in the 'future' according to the primary source of gravitation, the orbiting body may experience a change in displacement of $\dot{x} \approx 0$ while gravity propagates across the density axis $s$. 
...
Consider the elementary derivation of time dilation given in figure <FigRef id="basic-dilation" />. We've confirmed experimentally that an observer traveling between points $A$ and $B$ will arrive at point $B$ in a time equivalent to $vt^{\prime} = v t \gamma$.  

If two observers, $O$ and $O^{\prime}$ conspire that observer $O^{\prime}$ should travel at preciously velocity $v$ between points $A$ and $B$, at what time will $O^{\prime}$ reach $B$? If $O$ does not share the time dilation of $O^{\prime}$, how can effects like Lorentzian length contraction and transformation of velocities apply? More importantly, how can they apply in a manner in which $O^{\prime}$ travels at precisely the agreed upon velocity $v$? While one observer may tell another, _'travel at this velocity'_, how can one tell another _'experience this amount of time'_?
...
### 14. Charged particles in a gravitationally accelerated reference frame [#charged-particle-gravitational-accel]

As originally first addressed by Born in 1909<span className="citationAnchor-container no-tooltip"><a href='#bib-bornGravityChargedParticleParadox' className="citation citationAnchor" id="cit-bornGravityChargedParticleParadox-idx-4">5</a></span>, why should it be that an accelerated charged particle does not produce electromagnetic radiation when accelerated by a gravitational field in the same manner as when it is being accelerated in $\mathbb{R}^{3}$ space? After all, the equivalence principle dictates that these two accelerated reference frames should be entirely indistinguishable to a local observer.

If the electromagnetic field exists as medium within a larger $\mathbb{R}^{3}$ space that encompasses our expanding Universe, and it is the local spatial dilation that is in motion relative to this larger space, $\mathbb{R}^{3}_{\tau}$, the particle in a gravitationally accelerated reference frame would be at rest in regards to the larger $\mathbb{R}^{3}_{\tau}$ space while space itself is in motion. This further supports the mass independence of gravity discussed [here](#gravity-mass-independence), and lends further credence to the notion that gravity is not a force.




### 15. There does not exist a single experimental validation for the principle of relativistic time dilation
...
When the mechanism that produces the relativistic effects we observe without the need to dilate time also yields a peculiar velocity that coincides with direct observation, and the proportional dilation of space corresponds to $\Phi$ within 2 parts in 100,000, it's time to go back to the chalkboard.
