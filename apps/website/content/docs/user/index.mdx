...
If you do are starting from an environment without Node or a Node package manager installed, the easiest way to accomplish this is to download Node, and use `npm` to install `pnpm` by running the following command in your terminal.
...
And on windows, you'll have to follow the docs [here](https://learn.microsoft.com/en-us/powershell/scripting/learn/shell/creating-profiles?view=powershell-7.4). This method will allow the core app files to exist separately from the compiled app, ensuring that they remain available during rebuilds and updates.

Alternatively, you can run the build cli and wait for it to pause when it can't find the `appConfig.ulld.json` file. If the cli cannot find the configuration file, it will pause and give the opportunity to drop the `appConfig.ulld.json` file into the directory that the cli creates. 
...
Whew... I know that's a lot, but you're all set to generate you're now all set to generate your application.
    </Step>
...
Lastly, you'll need to run 2 commands inside of the folder you just created by cloning the repository. Follow the same technique you used to find the directory you used to first clone the repository, except this time, go ***into*** the `ulld` directory itself.
