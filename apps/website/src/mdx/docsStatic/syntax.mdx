...
You can also add definitions through your notes, which are then turned into a single dictionary that can be accessed either on it's own page or through the command palette.
...
The definition will appear in that note with a reasonable defintion style, and it will be automatically added to a global dictionary which can be access at the dedicated dictionary page, or through the command palette for quick reference.
