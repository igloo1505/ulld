...
While the example above is intentionally over simplified and almost entirely useless, we can demonstrate the truthy and falsy nature of Javascript. If we run include this component with the following syntax:

```mdx
<EOrPi valueIsSquared={"false"} value={2} /> value will return 4,
while <EOrPi valueIsSquared={false} value={2} /> will return 2.
```
...
_"Extending"_ types is almost universal among programming languages. If type `A` extends type `B`, then type `A` will have all of the same properties as `B`, and any extra properties that are unique to `A`. If `A` and `B` have a property in common, the property of `A` will overwrite that of `B`.  
...
For those familiar with **HTML** or **CSS**, most components extend the **HTML** attributes of the outer most element. However, be aware that while <LogoAsText /> relies on Tailwind for it's theme-ability, `.mdx` files are currently not evaluated for their `className` content.  

If you notice a bug with the way **HTML** attributes are handled, please submit a bug report on Github.
