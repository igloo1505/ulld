...
export default class AppendRouter extends Command {
    ...
    typeRegex: RegExp =
        /export\s+type\s+RouterType\s+=\s{(?<content>[^}]*)};?/gm;
    ...
    appendToObjectContent(
        ...
        if (objContent && matches?.[0]) {
            ...
            const newMatch = matches[0].replace(
                objContent,
                `
...
`,
    ...
    replaceType(content: string, importName: string): string {
        ...
        if (matches) {
            return this.appendToObjectContent(
                content,
                matches,
...
found: ${targetExists}`);
        ...
        if (RegExp(`^import\\s+${importName}`, "gm").test(targetContent)) {
            return console.log(
                "Router already added. If you think this was an error, try adjusting the key which the router is being appended to.",
            );
        }
...
`;
        ...
        this.log(
            "Success. Your router was added to the primary ulld router in the node_modules folder.",
        );
    }
