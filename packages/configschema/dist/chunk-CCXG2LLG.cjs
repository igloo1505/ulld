...
var _chunkUKQ47DHKcjs = require('./chunk-UKQ47DHK.cjs');


var _chunkJUH5FQQRcjs = require('./chunk-JUH5FQQR.cjs');


var _chunkKSAZPCJHcjs = require('./chunk-KSAZPCJH.cjs');
...
var zodRegexFieldToJsonSerializable = (b = []) => {
  return b.map((a) => {
    if (a instanceof RegExp) {
      return a.source;
    }
...
var appConfigSchema = _zod.z.object({
  ...
  ).default(_chunkM677MBJMcjs.defaultIgnoreFilePaths).transform(zodRegexFieldToJsonSerializable),
  ...
  noteTypes: _chunkUKQ47DHKcjs.documentTypeConfigSchema.array().default(_chunkBMKGPO6Qcjs.defaultNoteTypes).transform((x) => {
    let ids = x.map((l) => l.id);
    let data = x;
    for (const defaultNoteType of _chunkBMKGPO6Qcjs.defaultNoteTypes) {
      if (!ids.includes(defaultNoteType.id)) {
        data.push(_chunkUKQ47DHKcjs.documentTypeConfigSchema.parse(defaultNoteType));
      }
    }
    return data;
  }).describe(
  ...
  navigation: _chunkKSAZPCJHcjs.navigationConfigSchema,
  ...
  slots: _chunkTBWRDALScjs.pluginSlotSchema.default({}),
...
exports.zodRegexField = zodRegexField; exports.zodRegexFieldTransform = zodRegexFieldTransform; exports.zodRegexFieldToJsonSerializable = zodRegexFieldToJsonSerializable; exports.appConfigSchema = appConfigSchema; exports.appConfigSchemaTransform = appConfigSchemaTransform; exports.appConfigDeepPartial = appConfigDeepPartial;
//# sourceMappingURL=chunk-CCXG2LLG.cjs.map