...
var appConfigSchema = _zod.z.object({
  ...
  noteTypes: _chunkCIWR5AEHcjs.documentTypeConfigSchema.array().default(_chunkBMKGPO6Qcjs.defaultNoteTypes).transform((x) => {
    let ids = x.map((l) => l.id);
    let data = x;
    for (const defaultNoteType of _chunkBMKGPO6Qcjs.defaultNoteTypes) {
      if (!ids.includes(defaultNoteType.id)) {
        data.push(_chunkCIWR5AEHcjs.documentTypeConfigSchema.parse(defaultNoteType));
      }
    }
    return data;
  }).describe(
  ...
  navigation: _chunkL4SEUS5Gcjs.navigationConfigSchema,
...
//# sourceMappingURL=chunk-KVIXCEBQ.cjs.map