...
var targetPath = path__default.default.join(
  __dirname,
  "../defaults/generatedDefaultConfigs/base.json"
);
var physicsMathPath = path__default.default.join(
  __dirname,
...
for (const k of configs) {
  if (k.config) {
    let exists = fs__default.default.existsSync(k.path);
    if (!exists) {
      throw new Error(
        `parsedBaseConfig targetPath does not exist at ${k.path}`
      );
    }
    fs__default.default.writeFileSync(k.path, JSON.stringify(k.config, null, 4), {
      encoding: "utf-8"
    });
