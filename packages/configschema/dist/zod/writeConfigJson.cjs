...
var writeConfigJson = async (config, targetDirectory) => {
  ...
  const jsonSchema = zodToJsonSchema.zodToJsonSchema(appConfigSchema, {
    name: "UlldAppConfig",
    "$refStrategy": "relative",
    "basePath": [targetDirectory]
  });
  await fs__default.default.promises.writeFile(path__default.default.join(targetDirectory, "appConfig.ulld.json"), JSON.stringify(parsed), { encoding: "utf-8" });
  return await fs__default.default.promises.writeFile(path__default.default.join(targetDirectory, "jsonSchema.ulld.json"), JSON.stringify(jsonSchema), { encoding: "utf-8" });
};
