...
var _chunkZLIBX2HScjs = require('./chunk-ZLIBX2HS.cjs');
...
var appConfigSchema = _zod.z.object({
  ...
  noteTypes: _chunkIB2BTZA2cjs.documentTypeConfigSchema.array().default(_chunkBMKGPO6Qcjs.defaultNoteTypes).transform((x) => {
    let ids = x.map((l) => l.id);
    let data = x;
    for (const defaultNoteType of _chunkBMKGPO6Qcjs.defaultNoteTypes) {
      if (!ids.includes(defaultNoteType.id)) {
        data.push(_chunkIB2BTZA2cjs.documentTypeConfigSchema.parse(defaultNoteType));
      }
    }
    return data;
  }).describe(
  ...
  navigation: _chunkZLIBX2HScjs.navigationConfigSchema,
...
//# sourceMappingURL=chunk-ORK3XHM3.cjs.map