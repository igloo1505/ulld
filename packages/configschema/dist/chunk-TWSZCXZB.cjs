...
var appConfigSchema = _zod.z.object({
  ...
  noteTypes: _chunkGV6X5BBPcjs.documentTypeConfigSchema.array().default(_chunkBMKGPO6Qcjs.defaultNoteTypes).transform((x) => {
    let ids = x.map((l) => l.id);
    let data = x;
    for (const defaultNoteType of _chunkBMKGPO6Qcjs.defaultNoteTypes) {
      if (!ids.includes(defaultNoteType.id)) {
        data.push(_chunkGV6X5BBPcjs.documentTypeConfigSchema.parse(defaultNoteType));
      }
    }
    return data;
  }).describe(
  ...
  navigation: _chunkC4XI6WMKcjs.navigationConfigSchema,
  UI: _chunkOHVUFY6Lcjs.mainUIConfigSchema,
...
//# sourceMappingURL=chunk-TWSZCXZB.cjs.map