...
var appConfigSchema = _zod.z.object({
  ...
  noteTypes: _chunkIDPXUFRGcjs.documentTypeConfigSchema.array().default(_chunkBMKGPO6Qcjs.defaultNoteTypes).transform((x) => {
    let ids = x.map((l) => l.id);
    let data = x;
    for (const defaultNoteType of _chunkBMKGPO6Qcjs.defaultNoteTypes) {
      if (!ids.includes(defaultNoteType.id)) {
        data.push(_chunkIDPXUFRGcjs.documentTypeConfigSchema.parse(defaultNoteType));
      }
    }
    return data;
  }).describe(
  ...
  navigation: _chunkEEXWFUX2cjs.navigationConfigSchema,
...
//# sourceMappingURL=chunk-KWIRSSW7.cjs.map