...
export const jupyterConfigSchema = z.object({
    ...
    environment: z.string().default(d.jupyter.environment).describe("The *absolute* path to the python environment with which to open Jupyter cells and notebooks."),
    ...
    jupyterPort: z.number().int().default(d.jupyter.jupyterPort).describe("Port on which JupyterServer instance is running. This value must match the value in your local jupyter server config."),
    ...
    kernel: z.string().default(d.jupyter.kernel).describe("Kernel name to use. Can be overriden with the search param ?kernel=someKernelName"),
