...
import BibSettingsSection, {
    BibSettingsSectionProps,
} from "./bib/BibSettingsSection";
...
const SettingsPageContent = ({
    ...
    const form = useForm<SettingSchemaType>({
        ...
        defaultValues: {
            ...settings,
            tooltips:
                typeof settings?.tooltips === "boolean" ? settings.tooltips : false,
            title: settings?.title || "Uh Little Less Dum",
            summary_showCitations:
                typeof settings?.summary_showCitations === "undefined"
                    ? true
                    : settings.summary_showCitations,
            summary_showTags:
                typeof settings?.summary_showTags === "undefined"
                    ? true
                    : settings.summary_showTags,
    ...
    const lockCurrentImage = async () => {
        ...
        toast({
            description:
                "The home page image will not change until this setting is changed.",
        });
    };
    ...
    const clearLockedImage = async () => {
        ...
        toast({
            description:
                "The landing page image will now change automatically once per day.",
        });
    };
        ...
        <Form {...form}>
            <div className={"text-gray-900 dark:text-gray-200 space-y-6"}>
                ...
                <div className={"flex flex-col justify-center items-start gap-4"}>
                    <legend
                        className={
                            "text-sm font-semibold leading-6 text-gray-900 dark:text-gray-200"
                        }
                    >
                        UI Settings
                    </legend>
                ...
                <div className={"flex flex-col justify-center items-start gap-4"}>
                    ...
                    <LabeledCheckbox
                        ...
                        onChange={() =>
                            form.setValue("cleanOnSync", !form.getValues("cleanOnSync"))
                        }
                        desc={
                            "Should we remove data from the database that is no longer in use?"
                        }
                        name="cleanOnSync"
                    />
                </div>
                <div className={"w-fit flex flex-col gap-3 justify-center items-start"}>
                    ...
                    />
                    {formData.lockedLandingImage === null ? (
                        <Button onClick={lockCurrentImage}>Lock Image</Button>
                    ) : (
                        <Button variant="secondary" onClick={clearLockedImage}>
                            Clear Locked Image
                        </Button>
                    )}
                </div>
