...
export const PdfViewer = ({ file, allowToc, noResponsive, pageWidth, onPageLoad, textRenderer, forcePage, onRender, withIcon, viewState, setViewState, tocContainerRef, canvasRef }: Props) => {
            ...
            {fileBlob && (
                <>
                    <Document
                        ...
                        className={"group/pdfDocument relative w-fit h-fit max-w-[min(768px,calc(100vw-4rem))] group-[.pdf-as-grid]/pdfContainer:max-w-[100vw]"}
                    >
                        {withIcon && <Link href={{
                            pathname: "/pdf",
                            query: { file }
                        }}
                            className={"absolute top-3 transition-all duration-300 right-3 z-50 bg-primary rounded-md scale-0 opacity-0 group-hover/pdfDocument:scale-100 group-hover/pdfDocument:opacity-100"}

                        >
                            <DynamicIcon name={"paperPdf"} className={"bg-primary text-primary-content m-2"} />
                        </Link>}
                        {!forcePage && <PdfToc numPages={numPages} tocContainerRef={tocContainerRef} activePage={pageNumber} />}
                        {viewState && Boolean(viewState.indexOf("allPages") >= 0 || viewState.indexOf("asGrid") >= 0) ? (<PdfViewAllPages asGrid={viewState.indexOf("asGrid") >= 0} setPage={handlePageFromViewAll} numPages={numPages} />) : (
                            <PdfPage
                                pageWidth={width}
                                pageNum={pageNumber}
                                forcePage={forcePage}
                    ...
                    {!forcePage && <div className={"flex-col justify-center items-center gap-2 hidden group-[.pdf-with-navigation]/pdfContainer:flex group-[.pdf-annotating]/pdfContainer:flex"}>
                        <p>
