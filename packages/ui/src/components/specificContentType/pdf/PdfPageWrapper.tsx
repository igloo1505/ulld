...
const PdfStateButton = ({ stateClass, setState, activeState }: ButtonProps) => {
    ...
    return (
        <Button
            className={clsx(stateClass === "toc" && "group-[.pdf-view-state-asGrid]/pdfPageWrapper:hidden group-[.pdf-view-state-allPages]/pdfPageWrapper:hidden group-[.pdf-view-state-annotating]/pdfPageWrapper:hidden",
                isActive() ? "bg-muted text-muted-foreground" : "bg-primary text-primary-foreground",
            )}
...
const PdfPageWrapper = ({ file }: PdfPageWrapperProps) => {
    const [pdfViewState, setPdfViewState] = useState<PdfClassKey[]>(["withNavigation"])
    ...
    const handleStateChange = (_class: PdfClassKey) => {
        ...
        if (inState) {
            if (_class === "annotating" || _class === "allPages" || _class === "loading") {
    ...
    const manager = useMemo(() => new PdfManager(file, canvasRef), [])
    ...
    const annotationRenderer = useCallback((item: any) => {
    ...
    return (
        <div className={clsx("flex flex-col justify-start items-center gap-4 p-6 group/pdfPageWrapper", `pdf-view-state-${pdfViewState}`)}>
            <div className={"flex flex-row justify-center items-center gap-4"}>
                {Object.keys(nameMap).map((k) => {
                    return <PdfStateButton stateClass={k as keyof WithoutLoading} setState={handleStateChange} activeState={pdfViewState} key={`pdf-view-state-${k}`} />
                })}
            </div>
            <div className={clsx("grid grid-cols-[auto_1fr_auto] h-full w-fit group/pdfContainer gap-4 lg:gap-8", getClasses(pdfViewState)
            )} ref={pdfGroupRef}>
                <TocContainer ref={tocContainerRef} />
                <div className={clsx("pdfview max-w-[100%-152px]")} >
                    <PdfSuspended noResponsive allowToc={true} canvasRef={canvasRef} textRenderer={annotationRenderer} file={file} tocContainerRef={tocContainerRef} viewState={pdfViewState} setViewState={handleStateChange} />
                </div>
                <PdfAnnotationContainer manager={manager} pdfGroupRef={pdfGroupRef} canvasRef={canvasRef} resetSelection={resetSelections} show={pdfViewState.includes("annotating")} searchText={searchText} setSearchText={setSearchText} />
            </div>
        </div>
    )
}
