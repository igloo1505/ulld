...
export const DialogWithForm = <T extends FieldValues>({ children, grow, onOpenChange, form, open, title, desc, isModal, onSubmit, btnLabel, className, withButtons }: DialogWithFormProps<T>) => {
    ...
    return (
        <Dialog
            open={open}
            onOpenChange={onOpenChange ? onOpenChange : (o) => {
                if (isModal && !o) {
                    router.back()
                }
            }}
        >
            <DialogContent className={clsx("max-w-[90vw] mdlg:max-w-screen-md", !grow && "w-fit md:w-fit")}>
                <DialogHeader>
                ...
                {withButtons !== false && <DialogFooter>
                    <a
                        role="button"
