...
const ifRequired = <T extends unknown>(
    isRequired: "all" | boolean | undefined,
    val: T,
    defaultVal: null | undefined = null,
) => {
    return isRequired === true || isRequired === "all" ? val : defaultVal;
};

export const fakeTaggableString = (maxWords: number = 3) => {
...
export function fakeMdxNote(rOpts: {
    latexTitle?: boolean;
    formatted?: boolean;
    summary?: boolean;
    imageSrc?: boolean;
    ...
    return {
        ...
        title: faker.lorem.words(5),
        latexTitle: rOpts.latexTitle ? faker.lorem.words(5) : undefined,
        rootRelativePath: faker.lorem.words(5),
        noteType: faker.lorem.words(5),
        content: faker.lorem.words(5),
        formatted: ifRequired(
            rOpts.formatted,
            faker.lorem.paragraphs(Math.floor(Math.random() * 12) + 1),
        ),
        summary: ifRequired(rOpts.summary, faker.lorem.paragraph()),
        ...
        sequentialIndex: ifRequired(rOpts.sequentialIndex, 1),
        remoteUrl: undefined,
        outgoingQuickLinks: faker.lorem.words(5).split(" "),
        quickLink: ifRequired(
            rOpts.quickLink,
            faker.word.noun({ length: { min: 6, max: 20 } }),
        ),
...
export function fakeDiet() {
    return {
        ...
        goals: faker.helpers.arrayElement([
            DIETARY_GOAL.weightLoss,
            DIETARY_GOAL.muscleGain,
            DIETARY_GOAL.autoImmune,
            DIETARY_GOAL.allergy,
            DIETARY_GOAL.gsd,
            DIETARY_GOAL.skin,
            DIETARY_GOAL.sexDrive,
            DIETARY_GOAL.anxiety,
            DIETARY_GOAL.memory,
            DIETARY_GOAL.energy,
            DIETARY_GOAL.adhd,
            DIETARY_GOAL.sleep,
            DIETARY_GOAL.depression,
            DIETARY_GOAL.mentalHealth,
        ] as const),
        lastUpdate: faker.date.anytime(),
    };
}
