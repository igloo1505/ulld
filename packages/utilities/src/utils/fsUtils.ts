...
export const getParentDirAndFilename = (p: string, ensureRootRelativeParent: boolean = true, fsRoot: string) => {
    return {
...
export const replacePrefix = (content: string, regex: string, replaceWith: string = "") => {
    return content.startsWith(regex) ? `${replaceWith}${content.split(regex)[1]}` : content
}

export const replaceAppendix = (content: string, regex: string, replaceWith: string = "") => {
