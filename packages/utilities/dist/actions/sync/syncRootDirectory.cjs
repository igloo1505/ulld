...
var chunkPKGVCOOI_cjs = require('../../chunk-PKGVCOOI.cjs');
...
var a=async()=>{let e=await(await fetch("/api/events/onSync",{method:"POST",body:JSON.stringify({offline:!navigator.onLine,removeIfNotInFs:!1,cleanBeforeSync:!1})})).json();if(e?.errorNotifications&&e?.errorNotifications?.length)for(let t of e.errorNotifications)useToast.toast(chunkPKGVCOOI_cjs.a[t.errorKey]);else e.success&&useToast.toast({title:"Success",description:"File system was synced with database."});return !0};
