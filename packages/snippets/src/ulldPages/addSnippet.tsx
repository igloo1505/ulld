...
const AddSnippetPage = ({ editing: item }: PageProps) => {
    const form = useForm<z.infer<typeof addSnippetSchema>>({
        resolver: zodResolver(addSnippetSchema),
        defaultValues: {
            content: item?.content || "",
            description: item?.description || "",
            keywords: item?.keywords ? ArrayUtilities.beArray(item.keywords) : [],
            keywordInput: "",
            language: item?.language
                ? (item.language as ShikiLanguage)
                : "typescript",
            ...(item?.id && { id: item.id }),
        },
    });
    ...
    return (
        <div
            className={
                "w-full flex flex-col-reverse md:grid md:grid-cols-2 md:gap-6 my-8 px-8 md:px-16"
            }
        >
