...
export class PluginPage extends ShellManager {
    constructor(data, pluginName, pageIndex, paths, parentSlotKey, subSlotKey) {
        ...
        this.shouldUse = true;
        this.importName = "GeneratedPageComponent";
        this.formattedImportName = "GeneratedPageComponent";
        this.haveModifiedImportName = false;
        this.allowImportPageProps = false;
        if (data.slot && parentSlotKey) {
            this.slotData =
                slotStaticData[parentSlotKey]?.[data.slot];
        }
    ...
    getTargetUrl() {
        ...
        let protectedPathData = buildData.protectedPaths.find((q) => q.pageFor.slot === this.parentSlotKey &&
    ...
    writeToFile() {
        if (!this.shouldUse) {
            return this.log(`Not writing page ${this.targetUrl} from plugin ${this.pluginName}. There was an issue with their configuration.`);
        }
        if (this.subSlot) {
            this.logWriteToFile();
