...
export class UlldBuildProcess extends Prompter {
    constructor(targetDir, branch = "main") {
        ...
        this.slotConflicts = [];
        this.pageConflicts = [];
        this.componentImportMap = {};
        this.alreadyProvidedPackageManager = false;
        this.git = new GitManager(targetDir, this.branch);
        this.isLocalDev = process.env.LOCAL_DEVELOPMENT === "true";
        this.applicationDir = path.join(targetDir, appData.templateRepo.buildDirName);
        ...
        this.packageJson = new TargetPackageJson(this.applicationDir, this.isLocalDev, this.branch);
        let additionalSources = new AdditionalSources(this.paths);
        let globalAppConfig = additionalSources.getAppConfig();
        this.appConfig =
            globalAppConfig ||
                new UlldAppConfigManager(this.applicationDir, this.isLocalDev, this.paths);
    ...
    async gatherPlugins() {
        this.log(`Gathering ${chalk.hex("#0ba5e9")("U")}LLD plugins from your appConfig...`);
        if (!this.appConfig.config) {
            this.logError("No appConfig was found. Can not continue.");
            throw new Error(`No app configuration was found during the gatherPlugins phase.`);
        }
        let newPlugins = [];
        let fromConfigPlugins = this.appConfig.config?.plugins?.map((c) => new UlldPlugin(this.paths, c.name, c.version, this.branch)) || [];
...
`);
    ...
    applyPluginsToPackageJson() {
        this.log(`Generating a package.json file specific to your configuration...`);
...
and continue when that file is in place.`,
            ...
            if ("addedConfig" in shouldContinue && shouldContinue.addedConfig) {
                ...
                if (!nowValid) {
                    log("We still can't find your config. Please look over the docs " +
    ...
    async resolvePageConflicts() {
        if (this.slotConflicts.length > 0) {
            ...
            this.log(this.slotConflicts.length === 0
        ...
        for (const plugin of p) {
            for (const component of plugin.components) {
                if (component.subSlot) {
                    ...
                    slotMap[component.subSlot.parentSlot][component.subSlot.subSlot].push(component.subSlot);
                }
            }
        }
        for (const w in slotMap) {
            for (const z in slotMap[w]) {
                if (slotMap[w][z].length > 1) {
                    this.slotConflicts.push(new SlotConflict(w, z, slotMap[w][z]));
                }
            }
        }
    }
    ...
    removeUnusablePlugins() {
        ...
        if (pluginData.unusable.length === 0) {
            this.log(`Checked for issues with your plugins and found none! Continuing with the build process.`);
        }
        else {
            ...
            if (Array.isArray(itemData)) {
                for (const slotItem of itemData) {
                    if (!currentPluginNames.includes(slotItem.name)) {
                        this.plugins.push(new UlldPlugin(this.paths, slotItem.name, slotItem.version, this.branch));
    ...
    getComponentSlotMap() {
        ...
        let slotComponentMap = generateSlotMapOfType((slotKey, subSlotKey) => {
            let slotPage = slotPages.find((f) => f.subSlotKey === subSlotKey && f.parentSlotKey === slotKey);
            if (slotPage) {
                return slotPage;
            }
            return (slotComponents.find((f) => f.slotKey === slotKey && f.subSlotKey === subSlotKey) || false);
        });
...
${fullSlotMap.missingItems.map((k, i) => `${i + 1}. ${k.slot} -> ${k.subSlot}`).join("\n")}`);
    ...
    async applyPages() {
        for await (const k of this.plugins) {
            this.logError(`Attempting to apply extra pages. This method is not yet handled. If you are a user, send me a strongly worded email about this...`);
            // await k.applyPages();
        }
    }
    getTailwindSources() {
