...
export class UlldAppConfigManager extends ShellManager {
    ...
    constructor(targetDir, isLocalDev, paths) {
        ...
        this.directoryConfigPathTemplate = path.join(targetDir, "appConfig.ulld.json");
        this.directoryConfigPath = {
            path: this.directoryConfigPathTemplate,
            exists: fs.existsSync(this.directoryConfigPathTemplate),
        };
        this.preferredConfig = Boolean(this.envConfigPath && this.envConfigPath.exists)
            ? "env"
            : "directory";
        this.exists = Boolean((this.envConfigPath && this.envConfigPath.exists) ||
...
`);
    ...
    getConfigPath() {
        ...
        if (this.directoryConfigPath?.path &&
            fs.existsSync(this.directoryConfigPath.path)) {
            return this.directoryConfigPath.path;
        }
    }
    getValidConfigPaths() {
        return [this.envConfigPath, this.directoryConfigPath].filter((j) => j?.exists);
    }
