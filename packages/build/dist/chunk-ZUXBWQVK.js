...
// src/classes/baseClasses/fileManager.ts
init_esm_shims();
import fs from "fs";
import path from "path";
var FileManager = class _FileManager extends ShellManager {
...
`);
  ...
  copyToFile(path2, throwIfNotExists = false) {
    let content = this.getContent();
    const output = _FileManager.fromAbsolutePath(path2, this.paths, false);
    if (throwIfNotExists) {
  ...
  static fromPathKey(pathKey, paths) {
    const path2 = paths[pathKey];
    return new _FileManager(paths.makeSubPath(path2), paths);
  }
  static fromAbsolutePath(path2, paths, isDir) {
    let item = new _FileManager("", paths, isDir);
    item.setPath(path2, isDir);
    return item;
  }
};
...
//# sourceMappingURL=chunk-ZUXBWQVK.js.map