...
export default class BuildUlldApp extends Command {
    ...
    static flags = {
        ...
        clean: Flags.boolean({
            ...
            aliases: ["fresh"],
            summary: "This command will remove an existing build if one exists, and set the output path to the path of the existing build.",
        }),
        genDatabase: Flags.boolean({
            description: "Generate initial database structure during build.",
            aliases: ["migrate"],
            summary: "This command should only be used when generating a ULLD app against an empty database unless otherwise indicated by a specific release's documentation.",
        }),
    };
    static args = {
        branch: Args.string({
