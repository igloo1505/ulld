...
import {
    getAutoSettingsWithRegex,
    getConfigAutoSettings,
} from "../../trpcInternalMethods/settings/autoSettings/getAutosettingWithRegex";
...
export const autoSettingRouter = router({
    ...
    getAutoSettings: publicProcedure
        .input(
            z.object({
        ...
        .query(async (opts) => {
            ...
            if (opts.input.withRegex) {
                return await getAutoSettingsWithRegex(
                    prisma,
                    appConfig,
                    opts.input.type,
                );
            ...
            if (!opts.input.withRegex) {
                let d: {
                    ...
                    value: string;
                    type: autoSetting;
                }[] = getConfigAutoSettings(appConfig);
    ...
    deleteAutoSettingById: publicProcedure
        .input(z.number().int())
    ...
    addAutoSetting: publicProcedure
        .input(AutoPropertyFormSchema)
        .mutation(async ({ input }) => {
            if (input.id) {
