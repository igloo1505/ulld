...
export const syncBib = async (appConfig: AppConfigSchemaOutput, bibId: number = 1) => {
    const bibpath = appConfig.bibPath;
    const f = new FileData(path.join(appConfig.fsRoot, bibpath));
    if (!f.exists()) {
        console.warn(
            `Could not find a bibliography .bib file at path ${bibpath}. Your bibliography will not sync until this is resolved. Make sure to add the appropriate "bibPath" field to your appConfig.ulld.json file.`,
        );
        return {
    ...
    const { citations, userDefined } = getFormattedCslCitation(
        content,
        appConfig,
    );
