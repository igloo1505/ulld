...
import {
    MdxNoteProtocol,
    prismaMdxNoteSummaryZodSchema,
    PrismaMdxNoteSummary,
    PrismaMdxNoteWithKeys,
    prismaMdxNoteSummaryZodSchemaPreOutput,
...
export class MdxNote extends MdxNoteProtocol {
    ...
    summary?: string | null;
    raw: string;
    floatImages?: boolean | null = false;
    ...
    constructor(props: MdxNotePropsInput) {
        super(props.rootRelativePath || "", ".mdx");
        let parsedProps = mdxNotePropsSchema.parse(props);
        this.id = parsedProps.id;
        this.title = parsedProps.title;
        this.latexTitle = parsedProps.latexTitle;
        this.raw = parsedProps.raw;
        this.floatImages = parsedProps.floatImages;
        this.formatted = parsedProps.formatted;
        this.citations = parsedProps.citations;
        this.noteType = parsedProps.noteType;
        this.topics = parsedProps.topics;
        this.subjects = parsedProps.subjects;
        this.tags = parsedProps.tags;
        this.importantValues = parsedProps.importantValues;
        this.quickLinkId = parsedProps.quickLinkId;
        this.href = parsedProps.href;
        this.citationsListOrder = parsedProps.citationsListOrder;
        this.outgoingQuickLinks = parsedProps.outgoingQuickLinks;
        this.rootRelativePath = parsedProps.rootRelativePath;
        this.equationIds = parsedProps.equationIds;
        this.isProtected = parsedProps.isProtected;
        this.sequentialKey = parsedProps.sequentialKey;
        this.sequentialIndex = parsedProps.sequentialIndex;
        this.remoteUrl = parsedProps.remoteUrl;
        this.noLog = parsedProps.noLog;
        this.saveFormatted = parsedProps.saveFormatted;
        this.firstSync = parsedProps.firstSync;
        this.lastSync = parsedProps.lastSync;
        /* if (_matter?.created) { */
    ...
    async parseCitations(content?: string) {
        ...
        this.citations = this.citations.concat(
            fr.map(
                (c) =>
                    new BibEntry(c?.id, { htmlCitation: c?.htmlCitation }, c.pageIndex),
            ),
        );
...
`;
    ...
    setNoteType(config?: ParsedAppConfig) {
        if (!this.rootRelativePath) return;
        this.noteType = getNoteTypeFromPath(this.rootRelativePath as string, config)
            ?.docType as DocTypes | undefined;
    }
    ...
    static async fromMdxString({
        ...
        docType,
        getBookmarkState,
    }: FromMdxStringParams): Promise<MdxNote> {
        const fm = matter(content);
        const _matter = fm.data as FrontMatterType;
        let dt = docType
            ? zodDocTypeInput.parse(docType)
            : getNoteTypeFromPath(rootRelativePath as string)?.docType;
        const rll = withForwardSlash(
            rootRelativePath ||
            getRootRelativePath(urlSlug as string, dt as DocTypes),
        );
        let note = new MdxNote(
            undefined,
            rll,
            dt as DocTypes,
            content,
    ...
    static fromPrisma(item: NonNullable<MdxNoteWithAll> | PrismaMdxNoteWithKeys) {
        let newNote = new MdxNote(
            item.id,
            item.rootRelativePath,
            item.noteType as DocTypes,
            item.content,
            item.formatted,
            item.subjects,
            item.title,
            ...
            item.lastSync,
            item.isProtected,
            item.citationsListOrder,
        );
        newNote.href = item.href;
        if (item.definitions) {
            newNote.definitions = item.definitions.map(
                (d) =>
                    new Definition({
                        label: d.label,
                        content: d.content,
    ...
    static asSummary(_item: PrismaMdxNoteSummary): MdxNote {
        ...
        let n = new MdxNote(
            ...
            "",
            item.subjects
                ? item.subjects.map((s) => new Subject(s.value))
                : undefined,
        );
        ...
        n.citations = item.citations
            ? item.citations.map((t) =>
                BibEntry.fromPrisma({ id: t.id as unknown as string }),
            )
            : [];
    ...
    static fromList(
        ...
        return n.map((b) =>
            b instanceof MdxNote ? b : MdxNote.fromPrisma(b as PrismaMdxNoteWithKeys),
        );
    }
}
