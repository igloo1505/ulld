...
const TaskManagerTable = ({
    ...
    useEventListener("add-new-task", (e) => {
        if( e.detail.toDoListId && activeListIds.some((x) => x === e.detail.toDoListId)){
            _setItems([
                ...items,
                e.detail
            ])
    ...
    const selectedIds = getSelected().map((r) =>
        r.getValue(tableIds.id),
    ) as number[];
