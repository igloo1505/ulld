...
export const EditableDataTableDateCell = ({
    label,
    value,
    onValueChange,
    formatString,
}: EditableDataTableDateCellProps) => {
    const formatDate = (
    ...
    const getDateValue = () => {
        ...
        let dateValue = formValues.value
            ? laziestSchemaEver.parse(formValues).value
            : undefined;
        if (!dateValue) return;
                ...
                <DialogTrigger asChild>
                    <div
                        ...
                        className={clsx(
                            "max-w-[200px]",
                            !valueString &&
                            "flex flex-row justify-start items-center gap-1 text-muted-foreground fill-muted-foreground hover:text-foreground hover:fill-foreground transition-colors duration-150",
                        )}
                    >
                        {valueString ? (
                            valueString
