...
const CommandPalette = ({ search, page: _page, loading, setLoading, pages, setOpen, setPages, setSearch }: CommandPaletteProps) => {
    ...
    return (
        <Command
            className={cn("command-palette", (typeof _page !== "undefined" && sideBySidePages.includes(_page)) ? "side-by-side relative" : "linear")}
            onKeyDown={(e) => {
        ...
        >
            ...
            <Command.List
                className={cn("no-scrollbar relative", (_page && sideBySidePages.includes(_page)) && "border-t")}
            >
