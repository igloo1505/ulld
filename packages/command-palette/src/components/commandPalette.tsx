...
import EquationSearchCommandGroup, {
    EquationCmdItem,
} from "./groups/equationSearch";
import DefinitionSearchCommandGroup, {
    DefCmdItem,
} from "./groups/definitionSearch";
import {
    sideBySidePages,
    pageMap,
    pages as pageList,
    pages,
} from "./commandPaletteUtilities";
...
const CommandPalette = ({
    ...
    const defaultCommandList = useMemo(() => {
        return getDefaultCommandList(appConfig);
    }, [appConfig]);
    ...
    const [equationSearchCommands, _setEquationSearchCommands] = useState<
        EquationCmdItem[]
    >([]);
    ...
    const [definitionSearchCommands, _setDefinitionSearchCommands] = useState<
        DefCmdItem[]
    >([]);
    ...
    const [todoListSearchCommands, _setTodoListSearchCommands] = useState<
        TodoListCmdItem[]
    >([]);
    ...
    const [noteSearchCommands, _setNoteSearchCommands] = useState<
        NoteSearchItem[]
    >([]);
    ...
    const [subjectSearchCommands, _setSubjectSearchCommands] = useState<
        SubjectSearchItem[]
    >([]);
    ...
    const gatherDefinitionCommands = async () => {
        setLoading(true);
        let definitionData = await client.commandPalette.getDefinitions.query({
            rsc: false,
        });
        setLoading(false);
        setDefinitionSearchCommands(
            definitionData.map((d) => ({
                ...d,
                label: d.label || d.id,
    ...
    > = {
        ...
        searchEquations: (dir) => {
            ...
            if (dir === "prev") {
                newIdx =
                    curIdx === 0
                        ? equationSearchCommandsRef.current?.length - 1
                        : curIdx - 1;
            } else {
                newIdx =
                    curIdx === equationSearchCommandsRef.current?.length - 1
                        ? 0
                        : curIdx + 1;
            }
        ...
        searchNotes: (dir) => {
            let curIdx = noteSearchCommandsRef.current
                ?.map((d) => d.value)
                .indexOf(valueRef.current);
            const tcur = typeof curIdx;
            if (tcur === "undefined" || tcur !== "number") {
            ...
            if (dir === "prev") {
                newIdx =
                    curIdx === 0 ? noteSearchCommandsRef.current?.length - 1 : curIdx - 1;
            } else {
                newIdx =
                    curIdx === noteSearchCommandsRef.current?.length - 1 ? 0 : curIdx + 1;
            }
            return noteSearchCommandsRef.current?.[newIdx]?.value;
        },
        searchDefinitions: (dir) => {
            let curIdx = definitionSearchCommandsRef.current
                ?.map((d) => d.value.toLowerCase())
                .indexOf(valueRef.current);
            const tcur = typeof curIdx;
            if (tcur === "undefined" || tcur !== "number") {
            ...
            if (dir === "prev") {
                newIdx =
                    curIdx === 0
                        ? definitionSearchCommandsRef.current?.length - 1
                        : curIdx - 1;
            } else {
                newIdx =
                    curIdx === definitionSearchCommandsRef.current?.length - 1
                        ? 0
                        : curIdx + 1;
            }
        ...
        searchSubjects: (dir) => {
            ...
            if (dir === "prev") {
                newIdx =
                    curIdx === 0
                        ? subjectSearchCommandsRef.current?.length - 1
                        : curIdx - 1;
            } else {
                newIdx =
                    curIdx === subjectSearchCommandsRef.current?.length - 1
                        ? 0
                        : curIdx + 1;
            }
        ...
        searchTags: (dir) => {
            let curIdx = tagSearchCommandsRef.current
                ?.map((d) => d.value)
                .indexOf(valueRef.current);
            const tcur = typeof curIdx;
            if (tcur === "undefined" || tcur !== "number") {
            ...
            if (dir === "prev") {
                newIdx =
                    curIdx === 0 ? tagSearchCommandsRef.current?.length - 1 : curIdx - 1;
            } else {
                newIdx =
                    curIdx === tagSearchCommandsRef.current?.length - 1 ? 0 : curIdx + 1;
            }
    ...
    useEffect(() => {
        let em = document
            .getElementById("cmd-palette-input-parent")
            ?.querySelector("input");
        if (em) {
    ...
    const getHref = (v: string): string => {
        ...
        if (pages.at(-1) === "searchNotes") {
            let idx = noteSearchCommands
                .map((n) => n.href.toLowerCase())
    ...
    let currentDefinitionMarkdown = definitionSearchCommands.find(
        (d) => `def-${d.id.toLowerCase()}` === value,
    );
    ...
    return (
        <Command
            className={cn(
                "command-palette",
                typeof _page !== "undefined" && sideBySidePages.includes(_page)
                    ? "side-by-side relative"
                    : "linear",
            )}
            onKeyDown={(e) => {
        ...
        >
            ...
            <Command.List
                className={cn(
                    ...
                    _page && sideBySidePages.includes(_page) && "border-t",
                )}
            >
                <div className={"cmdItemsPanel"}>
                    <div className={"cmdListLeft"}>
                        ...
                        <DefaultCommandPaletteItems
                            ...
                            close={close}
                            search={search}
                            defaultCommands={defaultCommandList}
                        />
