...
} from "@ulld/configschema/staticData/defaultImageMap";
...
const ImageMapImage = (props: WithRequired<EmbeddedImageProps, "image">) => {
    ...
    return (
        <div
            className={clsx(
                "px-3 !py-3 overflow-hidden [&_svg]:h-fit",
                fullWidth && "w-full flex justify-center items-center",
                props.block && "w-full",
                props.block && props.right && "flex flex-row justify-end items-center",
                props.block && props.center && "flex justify-center items-center",
                ...
                ) && "w-full flex justify-center items-center sm:w-fit",
                Boolean(
                    (props.left || props.right) &&
                    !props.block &&
                    Boolean(props.sm || props.small),
                ) && "w-full flex justify-center items-center xxs:w-fit",
                Boolean(
                    (props.left || props.right) &&
                    !props.block &&
                    Boolean(props.lg || props.large),
                ) && "w-full flex justify-center items-center md:w-fit",
                Boolean(
                    (props.left || props.right) && !props.block && Boolean(props.xl),
                ) && "w-full flex justify-center items-center mdlg:w-fit",
            )}
            style={{
