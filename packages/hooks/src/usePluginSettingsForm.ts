import type { PluginSettingsRecord, ToastConfig } from "@ulld/types";
...
> extends UpdatePluginSettingOptionsSchema {
  ...
  parseDataBeforeSync?: (data: Partial<T>) => J;
  toastOnSuccess?: ToastConfig;
}
...
// RESUME: Come back here tommorrow and make sure this works. Moved everything over from the navigation packages settings page, but still completely untested.
...
>(
  ...
  const updateFormData = (data: Partial<z.infer<T>>): void => {
    for (const k in data) {
      form.setValue(
        k as unknown as Path<z.TypeOf<T>>,
        data[k as keyof z.infer<T>],
      );
    }
  };
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    // eslint-disable-next-line -- Need to access updateFormData outside of the useEffect hook.
  }, []);
