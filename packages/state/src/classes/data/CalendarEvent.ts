...
export class CalendarEvent implements calendar_v3.Schema$Event {
    ...
    reminders?: {
        overrides?: { method?: "email" | "popup" | null, minutes?: number | null }[];
        useDefault?: boolean;
    } | null;
    ...
    color(): { backgroundColor: string, textColor: string, borderColor?: string } {
        if (this.visibility === "confidential") return { textColor: "hsl(var(--destructive-foreground))", backgroundColor: "hsl(var(--destructive))" }
        if (this.visibility === "private") return { textColor: "#000", backgroundColor: "#eab308" }
        if (this.visibility === "public") return { textColor: "#fff", backgroundColor: "#0ea5e9" }
        return { textColor: "hsl(var(--primary-foreground))", backgroundColor: "hsl(var(--primary))" }
    }
    getEditUrl() {
    ...
    toCalendarFormat(): EventInput {
        return {
            ...
            date: this.start?.date ? new Date(this.start.date) : this.end?.dateTime ? new Date(this.end.dateTime) : undefined,
